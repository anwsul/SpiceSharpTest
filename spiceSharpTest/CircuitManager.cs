...
public static class CircuitManager3 {
    ...
    static IComponent rheostat = new Rheostat("RH", 1000);
    ...
    public static void ConnectWire(string wireName, string nodeName, bool runSimulation = true) {
        ...
        Console.WriteLine($"Circuit: {wireName} is connected with {nodeName}");
    ...
    public static void TryRunningSimulation() {
        try {
            ...
            foreach (var violation in ex.Rules.Violations) {
                Console.WriteLine($"- Rule: {violation.Rule}");
                Console.WriteLine($"- Subject: {violation.Subject}");

                // Print specific violation type details  
                if (violation is VoltageLoopRuleViolation voltageLoop) {
                    Console.WriteLine($"  First node: {voltageLoop.First}");
                    Console.WriteLine($"  Second node: {voltageLoop.Second}");
                } else if (violation is FloatingNodeRuleViolation floatingNode) {
                    Console.WriteLine($"  Floating variable: {floatingNode.FloatingVariable}");
                    Console.WriteLine($"  Fixed variable: {floatingNode.FixedVariable}");
                }
            }
        } finally {
            Console.WriteLine($"------------------------------------------");
